...
const listAccessOneAsync = (selectedUser) => async (dispatch) => {
  try {
    let searchUserId = 0;
    ...
    const accessDataId = access.data.data[0].accessId;
    ...
    dispatch(listAccessOne(accessDataUpdated.data.data));
...
const accessOneSlice = createSlice({
  name: "accessone",
  initialState: [],
  reducers: {
    listAccessOne: (state, action) => {
      return action.payload;
    },
  },
});

export { listAccessOneAsync };
export const { listAccessOne } = accessOneSlice.actions;
export default accessOneSlice.reducer;
